...
class Face_Embedder:
    ...
    def extract_face_from_file(filename, required_size=(160, 160)) -> Face:
    ...
    def extract_face(pixels, required_size=(160, 160)) -> Face:
        # detect faces in the image
        results = Face_Embedder.__detector.detect_faces(pixels)
        if not results:
            return None

        # extract the bounding box from the first face only
        ...
        return Face(face_array, (x1,y1,x2,y2), embed)
